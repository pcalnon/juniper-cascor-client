...
| **Multi-module transition** | Moving from one major component to another (e.g., `client.py` → `ws_client.py` → `tests/`)                                  |
...
- **Be specific**: "Add retry logic to `CascorClient.submit_training_job()`" not "finish the retry work"
...
```bash
Continue implementing [FEATURE] in juniper-cascor-client.
...
- Run tests with: pytest tests/ -v --cov=juniper_cascor_client --cov-fail-under=80
- Type check: mypy juniper_cascor_client --strict
```

### Template: Debugging Session

```bash
Continue debugging [ISSUE DESCRIPTION] in juniper-cascor-client.
...
- Consumers affected: [JuniperCanopy/both]
```

### Template: Client API / Cross-Consumer Work

```bash
Continue [CLIENT API TASK] in juniper-cascor-client.

Completed:
- Modified juniper_cascor_client/client.py: [description]
- Updated juniper_cascor_client/ws_client.py if needed
- Updated juniper_cascor_client/exceptions.py if needed

Remaining:
- Implement [specific method or behavior change]
- Add/update unit tests in tests/
- Verify backward compatibility with consumers (JuniperCanopy)

Key context:
- REST client: CascorClient class in client.py
- WebSocket client: CascorTrainingStream in ws_client.py
- Exceptions: CascorClientError hierarchy in exceptions.py
- Default service URL: http://localhost:8200
- Run tests: pytest tests/ -v
- Run with coverage: pytest tests/ --cov=juniper_cascor_client --cov-report=term-missing --cov-fail-under=80
```
...
3. **Include the verification command** — Always tell the new thread how to check its work (`pytest tests/ -v`, `mypy juniper_cascor_client --strict`, etc.)
