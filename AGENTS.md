...
JuniperCascorClient          Synchronous REST client (context manager)
  ├── Constructor:           `_normalize_url` (APD-CCLIENT-005 / #129) — case-insensitive
  │                          scheme default, `hostname` (not `netloc`) required, trailing
  │                          `/` and `/v1` stripped. Until #129 merges, construction still
  │                          only `base_url.rstrip("/")`.
...
does not **case-insensitively** start with `URL_SCHEME_PREFIXES` (`http://`, `https://`;
RFC 3986 §3.1 — a case-sensitive check re-prefixes `HTTPS://host` into `http://HTTPS://host`)
→ reject an empty `hostname` (not `netloc`; a userinfo-only `http://user:secret@` has a
truthy netloc and a `None` hostname) with `JuniperCascorConfigurationError` → rebuild via
`urlparse` (scheme stored lowercase) → drop a trailing slash → strip a trailing `/v1`.

Until #129 merges, construction still only `base_url.rstrip("/")`. A schemeless host, a
`/v1`-suffixed origin (double `/v1` `api_url`), a hostless value, an uppercase `HTTPS://`
origin (silent TLS downgrade to hostname `https`), or a userinfo-only authority all
construct successfully and fail opaquely on the first request.
...
| `URL_SCHEME_*` | `URL_SCHEME_PREFIXES=('http://', 'https://')`, `DEFAULT_URL_SCHEME_PREFIX='http://'` | REST `base_url` normalisation (APD-CCLIENT-005 / #129). Matching is case-insensitive via `url.lower().startswith(...)`. WS streams do not use these. |
