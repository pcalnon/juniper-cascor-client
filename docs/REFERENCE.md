...
`JuniperCascorClient` builds every REST path as `api_url = f"{base_url}/v1"`. A schemeless host, a `/v1`-suffixed origin, or a hostless value used to construct silently and fail opaquely on the first request. [#129](https://github.com/pcalnon/juniper-cascor-client/pull/129) ports `_normalize_url` from the sibling clients (juniper-recurrence-client is the reference; the same netloc guard is `APD-DCLIENT-004` in juniper-data-client).

**Until #129 merges**, construction still only `base_url.rstrip("/")`. After it merges, `__init__` runs these steps in order:

1. Strip surrounding whitespace.
2. If the value does not start with `http://` or `https://` (`URL_SCHEME_PREFIXES`), prefix `http://` (`DEFAULT_URL_SCHEME_PREFIX`).
3. Parse with `urllib.parse.urlparse`. An empty `netloc` raises `JuniperCascorConfigurationError` (`base_url must include a host; got ...`). The typed error subclasses `JuniperCascorClientError` and carries no HTTP `status_code` — there was no response.
...
| `"http://example.com:9000/"` | `http://example.com:9000` | `http://example.com:9000/v1` |
| `""`, `"   "`, `"http://"`, `"https://"`, `"/v1"`, `"http:///v1"` | raises `JuniperCascorConfigurationError` (also catchable as the base) | — |
