...
Until #129 merges, construction still only `rstrip("/")` — the schemeless form and a `/v1` suffix fail opaquely (or hit `/v1/v1`) on the first request. `CascorTrainingStream` / `CascorControlStream` (`ws://...`) do not run this normalisation.
